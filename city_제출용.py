...
# 인구수 율곡동
# Dataset Import
kim2017 = pd.read_csv('population/population_2017.csv', encoding='cp949')
kim2018 = pd.read_csv('population/population_2018.csv', encoding='cp949')
kim2019 = pd.read_csv('population/population_2019.csv', encoding='cp949')
kim2020 = pd.read_csv('population/population_2020.csv', encoding='cp949')
kim2021 = pd.read_csv('population/population_2021.csv', encoding='cp949')
...
date_list_end_train = ['2017-05-20 16:00','2017-05-30 16:00','2017-06-19 18:00','2017-06-24 16:00','2017-07-01 10:00','2017-07-04 16:00','2017-07-06 16:00',
                        '2017-07-24 16:00','2017-07-28 16:00','2017-08-08 16:00','2017-08-12 16:00','2017-08-25 16:00',
                        '2018-06-03 16:00','2018-06-08 16:00','2018-06-25 16:00','2018-07-15 14:00','2018-08-22 16:00','2018-08-30 04:00',
                        '2019-05-26 16:00','2019-06-03 11:00','2019-06-06 15:00','2019-06-25 16:00','2019-07-05 16:00','2019-07-24 16:00','2019-08-14 16:00','2019-08-19 16:00','2019-09-10 16:00',
                        '2020-06-12 16:00','2020-06-15 16:00','2020-06-23 16:00','2020-07-08 18:20','2020-07-21 16:00','2020-08-05 16:00','2020-08-21 16:00','2020-08-31 16:30']
...
for iter in range(len(date_train)) :
    sample = pd.concat([sample, pd.Series(date_train[iter])], axis=0)

sample_ts = pd.DataFrame([])
for iter in range(len(date_test)) :
    sample_ts = pd.concat([sample, pd.Series(date_test[iter])], axis=0)

sample['warning'] = 'warning'
sample.columns = ['ds','warning']

sample_ts['warning'] = 'warning'
sample_ts.columns = ['ds','warning']

all = pd.DataFrame(pd.date_range('2017-01-01 01:00:00','2020-12-31 23:00:00', freq='1H'))
all['warning'] = 'not'
all.columns = ['ds','warning']

all_ts = pd.DataFrame(pd.date_range('2021-01-01 00:00:00','2022-01-01 00:00:00', freq='1H'))
all_ts['warning'] = 'not'
all_ts.columns = ['ds','warning']

comp_tr = pd.merge(all, sample, on='ds', how='left')
comp_tr.drop('warning_x',axis=1,inplace=True)
comp_tr.warning_y.fillna('not', inplace=True)

comp = pd.merge(all_ts, sample, on='ds', how='left')
comp.drop('warning_x',axis=1,inplace=True)
comp.warning_y.fillna('not', inplace=True)

train['warning'] = comp_tr.warning_y
test['warning'] = comp.warning_y
...
train_df = pd.read_csv('dataset/task1_sub_final_tr.csv', index_col=0)
test_df_xgb = pd.read_csv('dataset/task1_sub_final_ts.csv', index_col=0)


col = ['population', 'temp', 'rain', 'hum', 'week_weekday', 'holiday', 'st',
       'warning', 'y', 'ds']
train_df = train_df[col]

col2 = ['ds', 'y', 'population', 'temp', 'rain', 'hum', 'week_weekday',
       'holiday', 'st', 'warning']
test_df_xgb = test_df_xgb[col2]
